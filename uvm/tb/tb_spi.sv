// =============================================================================
// tb_spi.sv - Unit testbench for spi_master_apb
// Purpose : Connects the APB agent interface to the SPI DUT and runs the SPI
//           test. MISO is looped back from MOSI.
// Author  : <author placeholder>
// Date    : <date placeholder>
// =============================================================================
`timescale 1ns/1ps
`default_nettype none

module tb_spi;
    import uvm_pkg::*;
    `include "uvm_macros.svh"
    import soc_uvm_pkg::*;

    logic clk = 1'b0;
    logic resetn;
    always #5 clk = ~clk;

    apb_if apb (.clk(clk), .resetn(resetn));

    wire        spi_sclk, spi_mosi, irq;
    wire [3:0]  spi_cs_n;

    spi_master_apb dut (
        .clk(clk), .resetn(resetn),
        .psel(apb.psel), .penable(apb.penable), .pwrite(apb.pwrite),
        .paddr(apb.paddr), .pwdata(apb.pwdata), .pstrb(apb.pstrb), .pprot(apb.pprot),
        .prdata(apb.prdata), .pready(apb.pready), .pslverr(apb.pslverr),
        .spi_sclk(spi_sclk), .spi_mosi(spi_mosi),
        .spi_miso(spi_mosi),    // loopback
        .spi_cs_n(spi_cs_n), .irq(irq)
    );

    initial begin
        resetn = 1'b0;
        repeat (5) @(posedge clk);
        resetn = 1'b1;
    end

    initial begin
        uvm_config_db #(virtual apb_if)::set(null, "*", "vif", apb);
        run_test("spi_test");
    end

    initial begin
        #5ms;
        `uvm_error("TB_SPI", "global timeout reached")
        $finish;
    end
endmodule

`default_nettype wire
